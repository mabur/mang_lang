...
std::string serialize(const ExpressionPointer& expression_smart) {
    ...
    switch (type) {
        case CHARACTER: return serializeCharacter(*dynamic_cast<const Character *>(expression));
        case CONDITIONAL: return serializeConditional(*dynamic_cast<const Conditional *>(expression));
        case DICTIONARY: return serializeDictionary(*dynamic_cast<const Dictionary *>(expression));
        case NAMED_ELEMENT: return serializeDictionaryElement(ExpressionPointer{type, index});
        case WHILE_ELEMENT: return serializeDictionaryElement(ExpressionPointer{type, index});
        case END_ELEMENT: return serializeDictionaryElement(ExpressionPointer{type, index});
        case FUNCTION: return serializeFunction(*dynamic_cast<const Function *>(expression));
        case FUNCTION_DICTIONARY: return serializeFunctionDictionary(*dynamic_cast<const FunctionDictionary *>(expression));
        case FUNCTION_LIST: return serializeFunctionList(*dynamic_cast<const FunctionList *>(expression));
        case LIST: return serializeList(expression_smart);
        case LOOKUP_CHILD: return serializeLookupChild(*dynamic_cast<const LookupChild *>(expression));
        case LOOKUP_FUNCTION: return serializeLookupFunction(*dynamic_cast<const LookupFunction *>(expression));
        case LOOKUP_SYMBOL: return serializeLookupSymbol(*dynamic_cast<const LookupSymbol *>(expression));
        case NAME: return serializeName(*dynamic_cast<const Name *>(expression));
        case NUMBER: return serializeNumber(*dynamic_cast<const Number *>(expression));
