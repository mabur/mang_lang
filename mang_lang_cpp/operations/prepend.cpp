...
ExpressionPointer prependList(ExpressionPointer list, ExpressionPointer item) {
    auto new_list = ::prepend(::list(list), item);
    return makeList(std::make_shared<List>(list->range, ExpressionPointer{}, new_list));
}

ExpressionPointer prependString(ExpressionPointer string, ExpressionPointer item) {
