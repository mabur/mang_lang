...
std::string evaluate(std::string code) {
    ...
    const auto standard_library = evaluate(parse(STANDARD_LIBRARY).get(), built_ins.get(), log);
    return serialize(evaluate(parse(code).get(), standard_library.get(), log).get());
